...
*This version 2 of Chemacs is a work in progress, it introduces necessary breaking changes to be able to support early-init.el, and may not yet work out of the box without some extra effort. For a solid, stable version head see the "classic" [[https://github.com/plexus/chemacs][Chemacs 1]].*
...
Both =~/.emacs-profiles.el= and =~/.emacs-profile= can also be stored under =$XDG\_CONFIG\_HOME/chemacs= (typically =~/.config/chemacs=) as =$XGD\_CONFIG\_HOME/chemacs/profiles.el= and =$XDG\_CONFIG\_HOME/chemacs/profile= respectively.
